...
import com.tlp.vendspring.netutil.DialogUtil;
    ...
    public void onClick(View v) {
        switch (v.getId())
        {
            case R.id.btn_clear_inventory:
                new DialogUtil().showDialog(getApplicationContext(), "确定清空库存？", new DialogUtil.OnDialogSureClick() {
                    @Override
            ...
            case R.id.ms_btn_reset:
                new DialogUtil().showDialog(getApplicationContext(), "确定重置？", new DialogUtil.OnDialogSureClick() {
                    @Override
                    public void sureClick() {
                        for(MsReplenishmentAisleResult.DataBean dataBean:list){
