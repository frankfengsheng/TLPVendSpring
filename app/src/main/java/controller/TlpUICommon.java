...
public class TlpUICommon {
    ...
    private static TlpUICommon m_Instance = null;
    ...
    public static synchronized TlpUICommon getInstance() {
        if (null == m_Instance) {
            m_Instance = new TlpUICommon();
        }
        return m_Instance;
    }
        ...
        if (Configuration.ORIENTATION_PORTRAIT == TcnVendIF.getInstance().getScreenOrientation()) {
            if (TcnShareUseData.getInstance().isFullScreen()) {
                ...
                if (CountEveryPage == 10) {
                    ...
                    res = R.layout.tlp_layout_commodity_item_goods;
                }
            }
        } else {
            if (CountEveryPage == 8) {
    ...
    public int getPayLayout() {
        ...
        String m_ScreenInch = TcnVendIF.getInstance().getScreenInch();
        if (m_ScreenInch.equals(TcnCommon.SCREEN_INCH[1])) {
            res = R.layout.tlp_dialog_pay_small;
        } else {
            res = R.layout.tlp_dialog_pay_small;
        }
        return res;
