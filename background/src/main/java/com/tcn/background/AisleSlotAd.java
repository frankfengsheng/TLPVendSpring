...
public class AisleSlotAd extends TcnMainActivity implements OnClickListener,OnItemClickListener{

	private static final String TAG = "AisleSlotAd";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		TcnVendIF.getInstance().LoggerDebug(TAG, "AisleSlotAd onCreate()");
	...
	protected void onResume() {
		TcnVendIF.getInstance().LoggerDebug(TAG, "AisleSlotAd onResume()");
		super.onResume();
	}
	...
	private void initData() {
		TcnVendIF.getInstance().LoggerDebug(TAG, "AisleSlotAd initData()");
		if ((pics_path != null) && (!pics_path.isEmpty())) {
			pics_path.clear();
		}
		pics_path = TcnVendIF.getInstance().getAdvertPathList();
		m_image_count = TcnVendIF.getInstance().getAdvertCount();
		TcnVendIF.getInstance().LoggerDebug(TAG, "AisleSlotAd pics_path： "+pics_path+" m_image_count: "+m_image_count);
	...
	private void initAdapter() {
		...
		in = getIntent();
		String path = in.getStringExtra("pathAd");
...
//			ContentUtils.getToast(this, pics_path.size()+"");
	...
	public void onClick(View v) {
		...
		if (id == R.id.pics_confirm) {
			...
			in.putExtra("slotAd", pics_path.get(m_selectPosition));
			setResult(101, in);
			finish();
	...
	public void search(View v)
	{
		...
		if(getString(R.string.backgroound_cancel).equals(btnstr)){
			...
			TcnVendIF.getInstance().queryAdvertPathList(0);
			...
			if("".equals(str)||null==str){
				...
				if ((null != pics_path) && (!pics_path.isEmpty())) {
					...
					TcnVendIF.getInstance().queryAdvertPathList(1);
				}
	...
	class MyAdapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			holder.mText.setText(TcnVendIF.getInstance().getFileName(true,pics_path.get(position)));
	...
	private class VendListener implements TcnVendIF.VendEventListener {
		...
		//	TcnVendIF.getInstance().LoggerDebug(TAG, "VendListener VendEvent m_lParam2: "+cEventInfo.m_lParam2+" m_lParam1: "+cEventInfo.m_lParam1);
			switch (cEventInfo.m_iEventID) {
				case TcnVendEventID.QUERY_ADVERT_PATHLIST:
					if ((pics_path != null) && (!pics_path.isEmpty())) {
						pics_path.clear();
					}
					pics_path = TcnVendIF.getInstance().getAdvertPathList();
