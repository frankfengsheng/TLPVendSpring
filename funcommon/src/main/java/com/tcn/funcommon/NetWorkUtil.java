...
/**
 * 作者：Jiancheng,Song on 2016/5/27 23:53
 * 邮箱：m68013@qq.com
 */
public class NetWorkUtil {
    ...
    public static int getGsmSignalStrength(int asu) {
        int iconLevel;
        if (asu <= 2 || asu == 99) {
            iconLevel = SIGNAL_STRENGTH_NONE_OR_UNKNOWN;
        } else if (asu >= 12) {
            iconLevel = SIGNAL_STRENGTH_GREAT;
        } else if (asu >= 8) {
            iconLevel = SIGNAL_STRENGTH_GOOD;
        } else if (asu >= 5) {
            iconLevel = SIGNAL_STRENGTH_MODERATE;
        } else {
            iconLevel = SIGNAL_STRENGTH_POOR;
        }
    ...
    public int getEvdoLevel(int evdoDbm, int evdoSnr) {
        int levelEvdoDbm = 0;
        int levelEvdoSnr = 0;
    ...
    public static boolean download(String url, String filePath) {
        boolean bRet = false;
        if ((null == url) || (url.length() < 1) || (null == filePath) || (filePath.length() < 1)) {
            return bRet;
        }
        HttpURLConnection conn = null;
        try {
            String saveDirPath = filePath.substring(0,filePath.lastIndexOf("/"));
            if (!FileOperation.instance().createFolder(saveDirPath)) {
                return bRet;
            }
            ...
            if(200 == conn.getResponseCode()) {
                ...
                bRet = FileOperation.instance().readAsFile(in,filePath);
            }
        } catch (IOException e) {
            e.printStackTrace();
        } catch (Exception e) {
